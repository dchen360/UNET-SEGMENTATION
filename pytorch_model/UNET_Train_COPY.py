...
class UNET_Train:
    ...
    def model_training(self, numOfEpoch, model_name):
        ...
        for epoch in range(numOfEpoch):
            ...
            for ith_batch, batch_data in enumerate(self.train_dataloader):
                ...
                ## Image to GPU ##
                # outputs = Variable(outputs).to(self.device)

                # calculate the dice_loss of each batch
                dice_loss = dice_coeff(outputs, labels_batch)
                ...
                # the epoch_loss would be the sum of the dice_loss of each batch
                epoch_loss += dice_loss.item()  # .item() extracts dice_loss values as floats
                if self.verbose:
                    sys.stdout.write('\rBatch dice_loss (batch {:d}/{:d}): {:.3f}'.format(
                        ith_batch + 1, len(self.train_dataloader), dice_loss.item()))
                else:
            ...
            # train_loss is the averaged dice_loss for each image
            train_loss = epoch_loss / len(self.train_dataloader)
            if self.verbose:
                print('Average epoch {:d} training dice_loss: {:.3f}'.format(epoch + 1, train_loss))

            # Generate testing loss
            test_loss = self.model_testing(epoch)
            ...
            if self.verbose:
                print('Average epoch {:d} training dice_loss: {:.3f} testing dice_loss: {:.3f}'.format(epoch + 1,
                                                                                                       train_loss,
                                                                                                      test_loss))

                #########################################
            ## Save Model State: Epoch 10 ###
            if (epoch + 1) == 10:
                torch.save(self.model.state_dict(), 'model/%s_E100.pt' % model_name)
            ### Save Model State: Epoch 10 ###
            if (epoch + 1) == 20:
                torch.save(self.model.state_dict(), 'model/%s_E200.pt' % model_name)
            ## END of Epoch ###
    ...
    def model_testing(self, currEpochNum):
            ...
            for batch_indx, data in enumerate(self.test_dataloader):  # iterate through the data
                ...
                if batch_indx == 0:
                    ...
                    if currEpochNum == 0:
                        ...
                        print(images.shape)
                        imgGrid = torchvision.utils.make_grid(images[:8])
                        # torch.Size([8, 512, 512])
                        # imgGrid = imgGrid.expand(imgGrid.shape[0], 3, imgGrid.shape[1], imgGrid.shape[2])
                        print(imgGrid.shape)
                        imgGrid = imgGrid.astype(np.uint8)
                        print(imgGrid.dtype)
                        self.writer.add_image('Epoch-%s_Original' % (currEpochNum + 1), imgGrid)

                    ## Push images every 10 Epochs ##
                    if (currEpochNum + 1) % 10 == 0:

                        ## Grid of Outputs ##
                        imgGrid = torchvision.utils.make_grid(outputs[:8])
                        self.writer.add_image('Epoch-%s_Reconstruction' % (currEpochNum + 1), imgGrid)
